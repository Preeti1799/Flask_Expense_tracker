...
def home():
    ...
    if selected_month:
        try:
            query = query.filter(db.extract("month", Expense.date) == int(selected_month))
        except ValueError:
            flash("Invalid month filter.")
    if selected_date:
        try:
    ...
    return render_template("index.html", expenses=expenses, total=total, selected_month=selected_month, selected_date=selected_date)
...
def add_expense():
    if request.method == "POST":
        ...
        new_expense = Expense(user_id=current_user.id, date=date, amount=float(amount), category=category, description=description)
